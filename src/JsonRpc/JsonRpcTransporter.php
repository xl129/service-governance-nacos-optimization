...
namespace YuanxinHealthy\ServiceGovernanceNacosOptimization\JsonRpc;

use Hyperf\JsonRpc\RecvTrait;
...
class JsonRpcTransporter implements TransporterInterface
{
     ...
     * @var float
     */
    private mixed $connectTimeout = 5;

    /**
     * @var float
     */
    private mixed $recvTimeout = 5;
    ...
    public function getClient(): SwooleClient
    {
        ...
        return Context::set(
            ...
            retry(2, function () {
                ...
                $key = spl_object_hash($this) . '.Connection.Node';
                Context::set($key, $node);
    ...
    public function getHasNode(): ?Node
    {
        $key = spl_object_hash($this) . '.Connection.Node';
